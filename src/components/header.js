...
// added media query for responsive hight when window size in small
  ...
  height: ${({ isHome }) => (isHome ? '70vh' : '20vh')} h1 {
    img {
      height: 80px;
    }
  }
...
componentDidUpdate = prevProps => {
	...
	if (breakpoint > 636) {
		// if browser window is greater than 636 width than make animation otherwise not
...
render() {
	...
	return (
		...
		>
			...
			<HeaderContainer
				ref={wrapper_logo =>
					(this.wrapper_logo = ReactDOM.findDOMNode(wrapper_logo))
				}
				isHome={location.pathname === '/'}
			>
