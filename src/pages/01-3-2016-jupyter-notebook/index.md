---
path: '/Setup_Jupyter_Notebook_CentOS/RHEL7'
title: 'Setup Jupyter Notebook CentOS/RHEL7'
...
documents that contain live code, equations, visualizations and ect ect.
...
instead and it is a fairly simple process and doesn't take more than 5-10 minutes.
...
Right know it is running on http://127.0.0.0:8888 and are only  locally assessable, what we want to do is make it assessable from remote PC's as well.

To do that we need to generate a configuration file with the following command.
```
jupyter notebook --generate-config
```
you will see an output like this where the path for the configuration file will be defined.
````
...
>>> ctrl+z
```
Now that we have created a password, will we open the configuration file and start the configuration of jupyter notebook.
